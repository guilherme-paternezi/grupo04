...
def main():
    ...
    while option != QUIT:
        ...
        f = open('./{}.txt'.format('tweets' if query == '' else query), 'w')
        with io.open('{}.txt'.format('tweets' if query == '' else query), 'w', encoding='utf-8') as f:
            f.write(tweetsTreated)

        f = open('./tweetsWoTreatment.txt', 'w')
        with io.open('tweetsWoTreatment.txt', 'w', encoding='utf-8') as f:
